...
tar zxf openssl_${VERSION}.tar.gz -C "${CURRENTPATH}/src"
cd "${CURRENTPATH}/src/openssl-OpenSSL_${VERSION}"
for ARCH in ${ARCHS}
do
	...
	echo "Building openssl_${VERSION} for ${PLATFORM} ${SDKVERSION} ${ARCH}"
	echo "Please stand by..."
	export CC="${BUILD_TOOLS}/usr/bin/gcc -arch ${ARCH}"
	mkdir -p "${CURRENTPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk"
	LOG="${CURRENTPATH}/bin/${PLATFORM}${SDKVERSION}-${ARCH}.sdk/build-openssl_${VERSION}.log"
	set +e
...
echo "Building done."
echo "Cleaning up..."
rm -rf ${CURRENTPATH}/src/openssl_${VERSION}
echo "Done."
